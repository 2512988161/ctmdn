...
class CTMDNCell(nn.Module):
    ...
    def forward(self, x, h, mem_alpha=None, mem_beta=None):
        ...
        for _ in range(self.unfolds):
            ...
            # ht≈=σ(…)⊙h(0)+(1−σ(…))⊙h(…)
            # current_h = torch.sigmoid(f_total) * current_h + (1 - torch.sigmoid(f_total)) * activated_state
...
if __name__ == "__main__":
    ...
    # 实例化模型
    model = CTMDNClassifier(INPUT_SIZE, HIDDEN_SIZE, OUTPUT_SIZE, unfold=4)
