...
class CTMDNCell(nn.Module):
    ...
    def forward(self, x, h, mem_alpha=None, mem_beta=None):
        ...
        for _ in range(self.unfolds):
            ...
            # Tanh(Linear(Inp, h) + Linear(Memory))
            # f_total = torch.tanh(base_drive + memory_drive)
            # Sin^2 (Linear(Inp, h) + Linear(Memory))
            ...
            # dalpha = current_alpha
            # dbeta =  current_beta
            # current_alpha = current_alpha + self.delta_t * dalpha
            # current_beta = current_beta + self.delta_t * dbeta
...
class nmodeLTCCTMDNClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, unfold=6, n_synch=None):
        super(nmodeLTCCTMDNClassifier, self).__init__()
        self.hidden_size = hidden_size
        # 初始化核心单元 CT-MDN Cell
        self.cell = CTMDNCell(
            input_size, 
