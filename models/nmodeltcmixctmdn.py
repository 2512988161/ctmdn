...
class CTMDNCell(nn.Module):
    ...
    def forward(self, x, h, mem_alpha=None, mem_beta=None):
        ...
        for _ in range(self.unfolds):
            ...
            current_h = (current_h+self.delta_t*f_total*A_para)/(1+self.delta_t*(f_total+1/tau))
...
class nmodeLTCMIXCTMDNClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, unfold=6, n_synch=None):
        super(nmodeLTCMIXCTMDNClassifier, self).__init__()
        self.hidden_size = hidden_size
        # 初始化核心单元 CT-MDN Cell
        self.cell = CTMDNCell(
            input_size, 
