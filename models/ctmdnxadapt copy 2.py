...
import math
from models.utils import visualize_compact,visualize_compact_with_heat,visualize_mask_filling,visualize_mask_filling_comparison
class CTMDNXCell(nn.Module):
    ...
    def forward(self, x, h, mem_alpha, mem_beta):
        ...
        return current_h, current_alpha, current_beta, steps,torch.stack(h_stack,dim=1)
...
class CTMDNXADAPTClassifier(nn.Module):
    ...
    def forward(self, x, x_true):
        ...
        for t in range(T):
            inp = x[:, t, :]
            h, mem_alpha, mem_beta, steps,h_stack = self.cell(inp, h, mem_alpha, mem_beta)
            outputs.append(h)
            mean_steps += (steps+1)
        ...
        if self.training:
            # [Batch, unfold, n_sync(Hidden), Hidden]
            h_stacks = torch.stack(h_stacks, dim=2)
            # fold 纬度mean
            x_cpu = x.detach().clone().cpu()
            h_stacks_cpu = h_stacks.detach().clone().cpu()
            x_true_cpu = x_true.detach().clone().cpu()
            # visualize_compact(h_stacks_cpu.mean(-1).unsqueeze(-1))
            # visualize_compact_with_heat(x_cpu,h_stacks_cpu.mean(1))
            # # visualize_mask_filling(x_cpu,h_stacks_cpu.mean(1).mean(-1))
            # visualize_mask_filling(x_cpu.mean(-1).unsqueeze(-1),h_stacks_cpu.mean(1).mean(-1)) 
            visualize_mask_filling(x_true_cpu.mean(-1).unsqueeze(-1),-h_stacks_cpu.mean(1).mean(-1), save_name="true_filling_imputation.png") 
            visualize_mask_filling_comparison(x_cpu.mean(-1).unsqueeze(-1),x_true_cpu.mean(-1).unsqueeze(-1),-h_stacks_cpu.mean(1).mean(-1), save_name="pred_filling_imputation.png") 
            # breakpoint()
        ...
        #     h_stacks = torch.stack(h_stacks, dim=2)
        #     # fold 纬度mean
        #     h_stacks = h_stacks.mean(dim=1)
        #     x_cpu = x.detach().clone().cpu()
        #     h_stacks_cpu = h_stacks.detach().clone().cpu()
        #     visualize_compact_with_heat(x_cpu,h_stacks_cpu)
        #     breakpoint()
